...
<head>
    ...
    <title>@yield('title', 'CounselConnect') — Student Portal</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside id="sidebar"
            ...
            <div class="px-6 py-5 border-b border-gray-100">
                ...
                <p class="text-xs text-gray-400 mt-0.5">Student Portal</p>
            </div>
            ...
            <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('student.dashboard') }}"
                   ...
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                          {{ request()->routeIs('student.dashboard') ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                ...
                <a href="{{ route('student.appointments.index') }}"
                   onclick="closeMobileMenu()"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                          {{ request()->routeIs('student.appointments.*') ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                ...
                <a href="{{ route('student.sessions.index') }}"
                   onclick="closeMobileMenu()"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                          {{ request()->routeIs('student.sessions.*') ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                ...
                <a href="{{ route('student.referrals.index') }}"
                   onclick="closeMobileMenu()"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                          {{ request()->routeIs('student.referrals.*') ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                ...
                <a href="{{ route('student.profile.edit') }}"
                   onclick="closeMobileMenu()"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                          {{ request()->routeIs('student.profile.*') ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                    <div class="text-right hidden sm:block">
                        ...
                        <p class="text-xs text-gray-400">
                            {{ Auth::user()->studentProfile?->year_level ?? '' }}
                            {{ Auth::user()->studentProfile?->department ?? 'Student' }}
                        </p>
                    </div>
                    <div class="w-9 h-9 rounded-full bg-blue-600 flex items-center justify-center text-white text-sm font-semibold shrink-0 overflow-hidden">
                        @if(Auth::user()->studentProfile?->profile_picture)
                            <img src="{{ asset('storage/' . Auth::user()->studentProfile->profile_picture) }}"
                                 alt="{{ Auth::user()->name }}"
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 flex flex-col items-center gap-4">
            ...
            <div class="flex gap-3 w-full mt-1">
                <button type="button"
                        ...
                        class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-100
                         transition-colors cursor-pointer">
                    Cancel
                </button>
                <button type="button"
                        onclick="document.getElementById('logout-form').submit()"
                        class="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium bg-red-500 text-white hover:bg-red-600 
                        transition-colors cursor-pointer">
                    Yes, log out
                </button>
            </div>
